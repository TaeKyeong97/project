      ...
      <section class="x-body">
        ...
        <nav>
          <header style="min-width:288px">
            <div class="h-info">
              <h2 class="x-xtitle"><a href="project.op?pj_no=${oppm[0].pj_no}"><c:forEach var="oppm" items="${oppm}" varStatus="status">${oppm.pj_title}</c:forEach></a></h2>
            ...
            <div class="x-xgroup g${opgp.pjG_no}">
              ...
              <div class="x-xgroup-b">
                <div class="x-xg-info">
                  ...
                  <span>
                  <c:if test="${oppm[0].pj_admin.equals(myid)}"><a href="pgAdminSetting.op?pjgNo=${opgp.pjG_no}&&pjNo=?${param.pj_no}">팀장설정</a></c:if>
                  <c:if test="${oppm[0].pj_admin.equals(myid)}"><a href="#" onclick="projectPeckIn('${opgp.pjG_no}','<c:out value="${param.pj_no }"></c:out>','${pjgno2[status.index].peSize}')">설정</a></c:if>
                    <a href="#">정보</a>
                  </span>
                </div>
                <div class="x-xnotice">
        ...
        <h1>구분</h1>
        <article id="GpContent" style="width:74%; ">
        <c:choose>
	       	<c:when test="${param.gpContent.equals('GongYu')}">	
	       		<script>
	       		var typeA="GongYu";
	       		var pj_no="${param.pj_no}";
	       		var pjgp_no="${param.pjgp_no}";
	       		var page="${param.page}";
          ...
          <div class="x-g-pcon">
            <div class="x-g-pcon-t">
              <span>디자인 기획안</span>
              <span>18'.12.08 ~ 19'.03.04</span>
            </div>
            <div class="x-g-pcon-st">
              <span class="x-g-st">디자인 기획에 관한 전반적인 부분을 총괄 합니다.</span>
          ...
          <div class="x-g-pcon" style="border-color:#60657d;">
            <a href="#" style="line-height:190px; text-align:center; display:block;">+ 프로젝트 생성</a>
          </div>
          <!--//그룹별 프로젝트 내용-->
            	</c:otherwise>
       	</c:choose>
